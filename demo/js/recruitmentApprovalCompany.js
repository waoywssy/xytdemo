...
function getenterInfoforJump(){
	var EnterInfo=eval('(' + localStorage.jofaiinfo + ')');
	//获取名称
...
//	                 $("#clicktoPreiEnterInfo").cli
				   ...
				   if(EnterInfo.enterinfoQq!=undefined){
					   $("#enterinfoQqPeri").html("QQ:"+EnterInfo.enterinfoQq);
				   }
				   //微信
				   ...
				   if(EnterInfo.enterInfoWebAddress!=undefined){
					   $("#enterinfoWebAddressPeri").html("网址:"+EnterInfo.enterInfoWebAddress);
				   }
	               //公司福利显示
...
function togetJolisInfo(jolisId){
		$.ajax({
			...
			success:function(data){
				...
				if(jsonMap.code==200){
					var list = jsonMap.data;
					var jsonstr = JSON.stringify(list[0]);
					localStorage.jolisinfo=jsonstr;//设置值
					console.log(localStorage.jolisinfo);//获取值
					location.href=encodeURI("../html/teachJobFairPosition.html");
				}
			},
			error:function(){
				alert("访问服务器失败！");
